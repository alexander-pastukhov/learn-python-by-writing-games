...
from [OpenClipart](https://openclipart.org/). They are [public domain](https://creativecommons.org/publicdomain/zero/1.0/) and can be used and distributed freely.] that we will use for the game and unzip them into _Images_ subfolder. Also, grab the [exercise notebook](notebooks/07-Memory game.ipynb)!
...
::: {.program}
Put your code into [code01.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code02.py]{.program-filename}.
:::
...
::: {.program}
Put **position_from_index** into [utilities.py]{.program-filename}. Put updated code into [code03.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code04.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code05.py]{.program-filename}.
:::

## Card factory
You have the code to create one card but we need eight of them. This definitely calls for a function. Write a function (put it into [utilities.py]{.filename} to declutter the main file) that takes three parameters

1. a window variable (you need it to create PsychoPy stimuli),
2. a filename,
3. card position index,

and returns a dictionary, just like the one you created. You very much have the code, you only need to wrap it into a function and document it. Call function `create_card_path`^[This function will assume that you provide a full relative path to the file. Later on, we will create a version of this function that will append the folder information itself.] and use it in the main script to create `card` dictionary. Think about libraries you will now need to import in [utilities.py]{.filename}.

::: {.program}
Put [create_card_path]{.program-function} into [utilities.py]{.program-filename}. Put code into [code06.py]{.program-filename}.
:::

## Getting a list of files {#file-list}
...
Try this out in a Jupyter Notebook (do not forget to import the [os](https://docs.python.org/3/library/os.html#module-os) library). You should get a list of 8 files that are coded as [[r|l][index].png]{.filename}, where _r_ or _l_ denote a direction the chicken is looking. However, for our game we need only four images (4 × 2 = 8 cards). Therefore, we need to select a subset of them, e.g., four chicken looking to the left or to the right only. Here, let us work with chicken looking to the left, meaning that we need to pick only files that start with "l". To make this filtering easier, we will use a cool Python trick called [list comprehensions](https://docs.python.org/3/tutorial/datastructures.html#list-comprehensions).
...
## Getting list of relevant files {#get-relevant-files}
...
Create `create_card` function based on `create_card_path` so that it assumes that the `filename` parameter is just the filename without the folder information and, therefore, build the path by [join](https://docs.python.org/3/library/os.path.html#os.path.join) it with the folder name (defined as a constant in the this module!). You now need to drop the `"Images/"` in the value that you pass to it. Test that the code works as before!

::: {.program}
Create [create_card]{.program-function} in [utilities.py]{.program-filename}. Put updated code into [code07.py]{.program-filename}.
:::
...
Copy the code for building a duplicated list of filenames that you tested in Jupyter notebook to your main script ([code08.py]{.filename}). Then, use enumerate and list comprehension over enumerated duplicate filenames to create `cards` (plural, replacing your singular `card` variable) via `create_card` function you wrote earlier. Update your drawing code to loop over and draw all cards. If your default is `"side"` is `"back"`, things will look pretty boring. Change that to '"front"` to see their faces.

::: {.program}
Put your code into [code08.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code09.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code10.py]{.program-filename}.
:::
...
::: {.program}
Put your code into [code11.py]{.program-filename}.
:::
...
However, you need to be careful not do add a card more than once (this will mess up our "how many cards are face up" number). There are several ways to do this. Assuming that `icard` is the index of the card, which you computed via `position_to_index()` from mouse position, you can simply check whether this card `"side"` is `"front"`. Alternatively, you can check whether this card is already [in](https://docs.python.org/3/reference/expressions.html?highlight=list%20dictionary#in){#in-container} the `face_up` list. Either way will tell you whether the card is face up. If it is not, you should set its `"side"` to `"front"` and add it to `face_up` list.

Implement this code, open a few cards. Then, use a breakpoint to pause the program and check that `face_up` list contains exactly these (this many) cards. If it has _more_ then your face-up checks do not work. Put a breakpoint on them and step through the code to see what happens.

::: {.program}
Put your code into [code13.py]{.program-filename}..
:::
...
* remove them from `face_up` list (see [.clear()](https://docs.python.org/3/tutorial/datastructures.html#more-on-lists){#list-clear} method).

::: {.program}
Put your code into [code14.py]{.program-filename}..
:::
...
::: {.program}
Put your code into [code17.py]{.program-filename}.
:::
